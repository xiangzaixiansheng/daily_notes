...
>  持有锁 **HOLDS THE LOCK(S):** `name`对应索引行值为`name_age`的记录锁写锁  **lock_mode X locks rec but not gap**
>  等待锁 **WAITING FOR THIS LOCK TO BE GRANTED:** 在插入意向锁之前等待间隙锁写锁释放 及**lock_mode X locks gap before rec insert intention waiting**
